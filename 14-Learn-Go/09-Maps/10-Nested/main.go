...
func getNameCounts(names []string) map[rune]map[string]int {
	runeMap := map[rune]map[string]int{}
	for _, name := range names {
		nameInitial := []rune(name)[0]
		if _, ok := runeMap[nameInitial]; !ok {
			runeMap[nameInitial] = map[string]int{}
		}
		if _, ok := runeMap[nameInitial][name]; !ok {
			runeMap[nameInitial][name] = 0
		}
		runeMap[nameInitial][name]++
	}
	return runeMap
}
